...
class TerminalFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwds):
        self.serial = serial.Serial(0, 115200)
        ...
        wxglade_tmp_menu.AppendSeparator()
        wxglade_tmp_menu.Append(ID_PROGRAM, "P&rogram device...", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.AppendSeparator()
    ...
    def __set_properties(self):
        ...
        #self.SetFont(wx.Font(8, wx.DEFAULT, wx.NORMAL, wx.NORMAL, 1, ""))
        self.text_ctrl_output.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL, 1, "Courier New"))
        # end wxGlade
    ...
    def __attach_events(self):
        ...
        self.Bind(wx.EVT_MENU, self.OnProgramDevice, id = ID_PROGRAM)
    ...
    def OnProgramDevice(self, event):
        ...
        if selection != '':
            try:
            ...
            except ProgramFileLoader.Error, e:
                dlg = wx.MessageDialog(None, e.error, e.caption, wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
            else:
                def sendWord(word):
                    """ Send word in network order (big-endian)"""
                    char = chr(word / 256)
                ...
                for c in range(len(pf.chunks)):
                    count, offset, data = pf.chunks[c]
                    msg = "Sending chunk #%d: %d words starting from %d" % (c+1, count, offset)
                    ...
                    for w in data:
                        ...
                        i += 1
                        if i % 512 == 0:
                            self.SetStatusText(msg + " (%d%%)" % (i*100/count))
                            
                self.SetStatusText("Sending done")
